...
export class ShopRegisterComponent implements OnInit {
    shopForm: FormGroup;
    constructor(private fb: FormBuilder,private router: Router,private shopService:ShopService,private http:HttpClient) {
     ...
     private fetchAddress(latitude: number, longitude: number) {
      ...
      this.http.get<any>(url).subscribe({
       ...
       next: (response) => {
        ...
        this.shopForm.value.address = response.display_name || 'Address not found';
       ...
       error: (err) => {
        ...
        alert('Could not fetch address. Please try again later.');
