  ...
  deleteRecord(id: number, name: string): void {
    ...
    this.shopService.deleteShop(id).subscribe({
      next: () => {
        ...
        this.getShops();
        this.toastr.success(`shop : '${name}' was deleted successfully!`);
      },
      error: (err) => {
        console.error('Error deleting shop', err);
      this.toastr.error('An error occurred while deleting the shop. Please try again.');
      },
    });
    // this.records.splice(index, 1);
  }
  ...
  verifyRecord(record: any): void {
    ...
    if (record.rating) {
      ...
      this.shopService
        ...
        .subscribe((res: any) => {
          this.toastr.success("seller successfully verified")
          console.log(res);
        });
      ...
      this.toastr.error("error verifing seller")
    }
